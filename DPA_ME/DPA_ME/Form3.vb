...
Public Class Form3
    'Name of disk file
    Private Const strFileName As String = "File3.txt"
    'Number of records
    ...
    'Input record from file at current position
    Private Sub ReadRecord()
        'Open an already-existing file for reading
        Dim FS As New FileStream(strFileName, FileMode.Open, FileAccess.Read)
        'Set the file pointer
        If intCurrRec = 1 Then 'if current record is 1 which is the first record at the start of the file it does not need the offset.
            FS.Seek((intCurrRec - 1) * intRecLen, SeekOrigin.Begin)
        Else
            FS.Seek((intCurrRec - 1) * intRecLen - intCurrRec + 1, SeekOrigin.Begin) ' off set to make sure things work redo this comment thanks matt ^^^^^^^^^^^^^^^^^^^^
        End If
    ...
    Private Sub OutputRecord()
        ...
        txtWhetherReceived.Text = strRecord.Substring(intStockIDLen + intDescriptionLen + intPriceLen + intQuantityInStockLen + intReorderLevelLen + intReorderQuantityLen + intDateLastOrderLen, intWhetherReceivedLen).Trim
